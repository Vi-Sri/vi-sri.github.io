---
layout: page
title: About Srinivas
kicker: Research engineer · independent writer
description: "I connect ideas across computation, evolution, morphogenesis, artificial intelligence, and complex adaptive systems."
permalink: /about/
---
<div class="about-lead">
  <img src="{{ '/assets/images/photo.jpg' | relative_url }}" alt="Portrait of Srinivas Venkatanarayanan" width="220" height="220">
  <div>
    <p class="lede">I am a Research Engineer at NVIDIA and an independent researcher interested in a stubborn question: how do local rules, acting without a central designer, produce robust form, adaptive behavior, and something that looks like mind?</p>
    <p>My route into that question runs through machine learning and computer vision, but the territory is broader: evolutionary computation, artificial life, information theory, statistical physics, morphogenesis, cellular automata, social systems, and philosophy of science.</p>
    <p>This site is where I make the route inspectable. The views here are personal and do not represent NVIDIA or any past employer.</p>
  </div>
</div>

## Experience

### Current

- **Research Engineer — NVIDIA** · Present

### Previous research and engineering roles

- **Imaging Research Intern — Williams-Sonoma** · San Francisco · 2025
- **Machine Learning Research Intern — Qualcomm** · San Diego · 2024
- **Graduate Research Assistant — Center for Research in Computer Vision, University of Central Florida** · Orlando · 2023
- **Lead Machine Learning Engineer — Toyota Connected** · Chennai · 2020
- **Lead Engineer, Centre of Excellence–AI — HCL Research** · Chennai · 2019
- **Machine Learning Engineer — Mad Street Den** · Chennai · 2019
- **Project Engineer, Computer Vision — Econ Systems** · Chennai · 2017
- **Member Technical Staff — Zoho** · Chennai · 2017

### Research consulting

- VideoVerse · 2024
- Voxela · 2023
- Omniaz · 2022
- Pavo & Tusker Innovations · 2018
- Hexr Factory · 2018

## What I am looking for

I welcome research collaborations built around a precise question, a reproducible artifact, and genuine room for disagreement. I am particularly interested in collaborators who can bridge formal models with biological or physical evidence.

[Email me](mailto:{{ site.author.email }}) · [GitHub](https://github.com/Vi-Sri) · [LinkedIn](https://www.linkedin.com/in/vishal-srinivas-9234703a/)

